    ...
    def __init__(self, data_dir, augment=True):
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith('.png'):
                # Extract genre from filename (e.g., 'classical00001.png' -> 'classical')
                genre_match = re.match(r'^([a-zA-Z]+)', filename)
                genre = genre_match.group(1) if genre_match else None
                if genre and genre in self.genre_to_idx:
                    # Getting image paths, file lables in numbers
...
if __name__ == "__main__":
    # Load datasets
    train_data = SpectrogramDataset(r"data copy/training", augment=True)
    val_data = SpectrogramDataset(r"data copy/validation", augment=False)
    ...
    results = train_with_early_stopping(model, train_data, val_data, 
                                       batch_size=32, num_epochs=10, lr=1e-3)
    
    # Test on testing data
    test_data = SpectrogramDataset(r"data copy/testing", augment=False)
    if len(test_data) > 0:
